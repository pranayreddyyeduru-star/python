...
#    a) For row `i`, print `i` numbers, so loop `j` from 1 to `i` 
# (inclusive).
#    b) Print the current value of `number` on the same line using 
# `end='  '`.
#    c) Increase `number` by 1 after printing so the next number continues
#  in sequence.

# 6) After finishing each row, print a blank `print()` to move to the next
#  line.
rows=int(input("Give me an integer: "))
num=1
